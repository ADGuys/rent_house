...
class UserLogin(APIView):  # 登陆
    ...
    def get(self, requests):
        ...
        user_obj = UserModel.objects.filter(account=account).values('password', 'user_id', 'type', 'name').first()
        ...
        if not user_obj:
            ...
            if user_obj['password'] == password:
                return Response(
                    {'info': {'user_id': user_obj['user_id'], 'user_type': user_obj['type'], 'name': user_obj['name']}})
            else:
